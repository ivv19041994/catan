...
`FCatanGameView` и посылают команды `TryBuildSettlement` / `TryBuildRoad`.
...
Следующий практический этап — Common UI HUD (фаза, игрок, ресурсы и доступные
действия), после него команды остальных фаз и только затем репликация.
